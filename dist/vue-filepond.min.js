...
/^on/.test(prop)?events.push(prop):// get property type ( can be either a String or the type defined within FilePond )
props[prop]={type:[String,(type=_filepond.OptionTypes[prop],{string:String,boolean:Boolean,array:Array,function:Function,int:Number,serverapi:Object,object:Object}[type])],
...
if(document.querySelector("#app").offsetParent){new MutationObserver(function(mutations,observer){var removedNode=((mutations[0]||{}).removedNodes||[])[0];removedNode&&removedNode.contains(document.querySelector("#app"))&&(observer.disconnect(),detached.call(_this3))}).observe(document.documentElement,{childList:!0,subtree:!0})}// if we're still attached it's likely a transition is running, we need to
